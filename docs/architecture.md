...
| 미연동 (명세 §11) | 엑셀 실파일 파싱, 배너 실이미지. ~~실제 카메라·QR 인식, 문자 게이트웨이~~ → 2026-07-17 qr-poc 이식으로 해소: html5-qrcode 실카메라 스캔 + 난수 qrToken(`/q/{token}` 패스) + SOLAPI 발송(`server/sms/`), 시드는 `server/seed/data.ts`(메모리·`pnpm db:seed` 공용) |
